...
- [x] **Step 1: Write `scripts/generate-formula.sh`**
...
- [x] **Step 3: Verify against a synthetic checksums file**
...
- [x] **Step 4: Verify the missing-checksum error path**
...
- [x] **Step 3: Sanity-check job wiring**
...
- [x] **Step 1: Update `docs/RELEASE.md`**
...
- [x] **Step 2: Update `AGENTS.md` Releasing section**
...
- [x] **Step 1: Add a `clj-pulse` section**
...
- [~] **Step 2: Commit and push the tap repo**
  ...
  *Done:* committed locally as `85b6e2d`. *Pending:* push awaits maintainer confirmation.
...
## Implementation summary (completed 2026-06-15)

**Status:** all code/docs tasks done. Two human-gated items remain (Task 5 + the tap push).

Commits on `master` (clj-pulse):
- `d5732da` — `scripts/generate-formula.sh` (Task 1)
- `aec3874` — `homebrew` job in `.github/workflows/release.yml` (Task 2)
- `71d7f22` — `docs/RELEASE.md` + `AGENTS.md` (Task 3)

Commit in `../homebrew-tap` (local only, **not pushed**):
- `85b6e2d` — `clj-pulse` section in `README.md` (Task 4)

Verification done:
- Generator produces a valid formula from synthetic checksums; maps the four
  Rust triples correctly; ignores the Windows artifact; errors (exit 1) on a
  missing file or missing checksum.
- `release.yml` parses as valid YAML; the `homebrew` job is wired
  `needs: release`, uses `HOMEBREW_TAP_TOKEN`, writes `tap/Formula/clj-pulse.rb`,
  and no-ops when unchanged.
- Each clj-pulse task passed a `review-with-codex` second-opinion review with no
  actionable findings.

Outstanding (human actions):
1. Push the `../homebrew-tap` commit `85b6e2d` (awaiting confirmation).
2. Add the `HOMEBREW_TAP_TOKEN` repo secret on clj-pulse (Task 5, Step 1).
3. Confirm the `homebrew` job end-to-end on the next release (Task 5, Step 2).

The formula file (`Formula/clj-pulse.rb`) is intentionally not seeded by hand;
the first one is generated by the next release run.
